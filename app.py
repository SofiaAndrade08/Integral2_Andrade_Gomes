...
@st.cache_resource
def cargar_modelo():
    ruta_modelo = Path(__file__).parent / "modelo_conectel.joblib"
    
    if not ruta_modelo.exists():
        raise FileNotFoundError(
            "No se encontró el archivo modelo_conectel.joblib en la carpeta de la aplicación."
        )
...
try:
    ...
    st.error(
        "No fue posible cargar el modelo. Verifica que el archivo "
        "`modelo_conectel.joblib` esté en la misma carpeta que `app.py`."
    )
    st.exception(e)
    st.stop()
...
with st.form("formulario_cliente"):
    ...
    st.caption("Si el cliente no tiene internet, la velocidad será considerada como 0 Mbps al generar la predicción.")
...
if boton_predecir:
     ...
     # Regla de consistencia: si no tiene internet, la velocidad debe ser 0
    if tiene_internet == 0:
        velocidad_mbps = 0
    ...
    # Ordenar columnas según entrenamiento y generar predicción con manejo de errores
try:
    columnas_faltantes = [col for col in columnas_modelo if col not in cliente.columns]
    
    if columnas_faltantes:
        raise ValueError(
            f"Faltan columnas requeridas por el modelo: {columnas_faltantes}"
        )
    ...
    # Predicción final según el umbral definido en el notebook
    prediccion = int(probabilidad_mora >= umbral_final)    

except Exception as e:
    st.error(
        "No fue posible generar la predicción. Revisa que los datos ingresados "
        "sean consistentes con las variables utilizadas por el modelo."
    )
    st.exception(e)
    st.stop()
